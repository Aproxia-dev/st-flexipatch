...
hbtransform(XftGlyphFontSpec *specs, const Glyph *glyphs, size_t len, int x, int y)
{
	...
	for (int i = 0; i < (LEN(features) - 1); i++) {
		...
		if (features[i][4] == '=') {
			...
			hbfeatures[i].value = 1;
		}
		hbfeatures[i].start = HB_FEATURE_GLOBAL_START;
		hbfeatures[i].end   = HB_FEATURE_GLOBAL_END;
...
hbtransformsegment(XftFont *xfont, const Glyph *string, hb_codepoint_t *codepoints, int start, int length)
{
	...
	/* Shape the segment. */
	hb_shape(font, buffer, hbfeatures, sizeof(hbfeatures)/sizeof(hb_feature_t));
